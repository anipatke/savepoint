...
- **Objective planning order:** `internal/data` decodes optional Objective priority and rank, provides the canonical order within a Goal, and reports duplicate ranks through doctor; this metadata does not change lifecycle, dependencies, or gates.
...
**Layout:** the V2 board uses an Objective sidebar grouped under non-empty `CRITICAL`, `HIGH`, `MEDIUM`, and `LOW` headings, followed by three Task columns (`planned`, `in_progress`, `done`), Goal selection scoped to the router's live Goal, focused detail overlays, static Atari-Noir surfaces, and a deterministic non-TTY plain table. Sidebar rows follow priority, rank, then stable ID order; Objectives with no priority or rank remain Medium and unranked, with ID as the fallback order. The plain output lists the selected Goal's Objectives after `Selected:` under the same headings and in the same order. The sidebar omits the separate Planned / In Progress / Done line beneath each Objective heading. Next is exactly the router's selected work plus the existing gate resolver for that record; no project-wide or Release-wide search substitutes other work. The columns open on the router's Objective, or, when the router selects an Issue alone, on the one Objective that Issue's linked Tasks and Objective-scoped Checks belong to; with no Objective filter, the board labels the view `ALL OBJECTIVES` for the selected Goal and the plain output names the selected Goal and its member Objectives. The board's Next area, non-TTY output, and the first line of `savepoint resume` print the same line:
