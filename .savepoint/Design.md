...
**Check code style review (O-023):** Task, Objective, and Goal detail overlays show `CODE STYLE (C-###)` after `CHECKS` when a latest Check exists. The section displays only that Check's authored `## Code Style Review` lines, including checkbox markers and reasons, or says that the Check has no Code Style Review. A record without a Check omits the section. This display does not affect clearance, badges, Next, or the non-TTY plain table.
