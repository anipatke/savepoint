...
- [x] Add defect storage to board model state
- [x] Extend board data loading through the existing dependency boundary
- [x] Add a compact defect summary renderer
- [x] Place the summary in existing header/Next Activity chrome without reducing column width
- [x] Add board rendering and layout regression tests
- [x] Run `make build && make test`

## Context Log

- Files read: model.go, interfaces.go, board.go, view.go, layout.go, board_test.go, layout_test.go, view_test.go, interfaces_test.go, watch.go, update.go, io.go, styles/styles.go, data/defect.go, data/discover.go, data/parser.go, testutil/fixture.go, testutil/fs.go
