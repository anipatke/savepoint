...
- A raw source inventory keyed by project-relative path with exact-byte SHA-256, size, and mode, taken from bytes rather than from parsed models, which is also the freshness basis: apply revalidates it and refuses when a source changed since the preview.
...
- Freshness tests prove apply revalidates raw source hashes and aborts with a named conflict when any inventoried source changed between preview and apply, without writing.
