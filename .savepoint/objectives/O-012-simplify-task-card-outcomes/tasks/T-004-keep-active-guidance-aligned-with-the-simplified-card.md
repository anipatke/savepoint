...
status: done
depends_on: [{task: T-005, requires: clear}]
owner_validation:
    required: false
    accepted_check: ""
check_waiver:
...
# T-004: Keep active guidance aligned with the simplified card
...
O-012's accepted vocabulary and confirm it does not redefine waiver or exception
as technical `CLEAR`. If the optional local Task Check is skipped, record an
explicit waiver and rely on O-012's mandatory Full Objective Check to verify
...
- Historical archives, immutable Checks, and Issues are left untouched;
  references there are treated as evidence, not active policy. O-900's fixture
  records are reconciled only by T-005 for visual coverage.
...
1. Treat T-003's tested renderer as the implementation authority and compare it
...
to statements made inaccurate by T-003.
...
to renderer/documentation agreement before O-012 can close.

## Technical Evidence

**Start gate:** T-005's `depends_on: [{task: T-005, requires: clear}]` was satisfied
before starting — T-005 is `status: done` with a recorded owner `check_waiver`
...
Confirmed by reading T-005's frontmatter directly (a Context File of this Task).

**Document inventory (all six named active documents reviewed):**

- Changed: `.savepoint/Design.md` — added a **Task-card review outcome (O-012)**
...
- Changed: `AGENTS.md` — the `internal/board/v2/` Codebase Map row's badge-
  ...
  current `taskReviewOutcomeBadge` (T-003) both take a `waived bool` input to
...
- Historical archives, immutable Checks, and Issues untouched: no file under
  ...
  read or written by this Task. O-900's fixture records (owned by T-005) were
  not touched.
...
- `go test ./internal/board/v2/...` — `ok` (cached from T-005's unmodified
...
- No fresh User Check or owner waiver has been requested/recorded for this
  ...
  evidence hands off to O-012's mandatory Full Objective Check rather than an
  optional local Task Check.
- The AGENTS.md badge-mapping omission of `waiver` predates T-003 (confirmed
  via `git log --oneline -- AGENTS.md`, whose most recent entry pre-O-012 is
  `e8ac64d`, and `git show fb68054 -- AGENTS.md`, which touched nothing in
  this file) — it was not literally "made inaccurate by T-003," but Done-When
