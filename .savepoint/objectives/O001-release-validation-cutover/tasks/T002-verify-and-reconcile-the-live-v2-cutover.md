...
- [x] O001 carries no Objective-owned blocker to R006's cutover decision. **Reworded 2026-09-21 (repairs I017)**: the prior wording of this criterion asked O001's own mandatory Full Objective Check to verify that R006 already passes `ResolveReleaseCutover` — but `ResolveReleaseCompletion` requires O001 itself to be `status: done` with current Objective clearance first (`internal/data/release_gate_v2.go`), and an Objective can only reach `done` after this very Check concludes and the owner closes it. As written, the criterion asked this Check to prove a fact that cannot exist until after the Check itself and the owner's later action — it was not satisfiable by any Objective Check, not just this one. `ResolveObjectiveCompletion` (`internal/data/objective_gate_v2.go`) does not itself read Release state at all: O001's own completion depends only on its owned Tasks being done and its own Check clearance being current. What this Task and Objective can actually prove, and now have: every T001/T002 acceptance criterion above is met, and `data.ResolveReleaseCutover` names exactly one blocker on R006 — `release_objective_incomplete` because O001 is not yet closed — which is not an O001-owned defect, only a precondition the owner and R006's own downstream, mandatory Release Check must still satisfy after this Objective Check. That downstream Release Check and exact owner acceptance remain required and are not shortcut by this criterion closing.
...
- 2026-09-21: **Defect repair (I017)**: the O001 Full Objective Check (C905) found the Release-cutover acceptance criterion above circular — it asked this Check to verify R006 already passes `ResolveReleaseCutover`, but `ResolveReleaseCompletion` requires O001 to already be `status: done` with current clearance, which cannot be true until after this same Check concludes and the owner closes O001. Confirmed against the actual resolvers rather than assuming the report: `ResolveObjectiveCompletion` (`internal/data/objective_gate_v2.go`) reads only owned-Task-done and O001's own Check clearance — it never reads Release state, so the circularity was a criterion-wording defect, not a gate-resolver defect. Reworded the criterion to state what this Check can actually prove now (no O001-owned blocker to R006's cutover) and to explicitly defer the Release-level pass and owner acceptance to R006's own mandatory, strictly-downstream Release Check. `data.ResolveReleaseCutover` still reports exactly one blocker on R006 — `release_objective_incomplete`, because O001 is not yet closed — which is correct and expected, not something this Task or its Check can resolve.
