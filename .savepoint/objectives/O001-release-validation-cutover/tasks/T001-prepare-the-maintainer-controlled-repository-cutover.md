...
- 2026-09-21: **Defect repair (I016)**: the O001 Full Objective Check (C905) found the rollback snapshot named above, `/tmp/savepoint-cutover-backups/0e7c9ed/`, no longer exists — `/tmp` is ephemeral and this deviated from the archived runbook's own documented location, `../savepoint-cutover-backups/<sha>/` (outside the project root, not under `/tmp`). Recreated the snapshot at the runbook-correct location, `/home/user/code/savepoint-cutover-backups/0e7c9ed/`: `repository.bundle` (`git bundle create --all`, verified with `git bundle verify` — complete history, contains commit `0e7c9ed`), `worktree.tar` (`git archive 0e7c9ed`, a deterministic reconstruction of that exact commit's tree rather than a live-directory copy), and `SHA256SUMS` over both plus `NOTE.md`, which records why the original `worktree.patch`/`index.patch` (uncommitted-diff-at-backup-time) cannot be reconstructed — no record of pre-commit uncommitted state survives, and commit `0e7c9ed`'s tree is itself a complete, self-consistent snapshot. `sha256sum -c SHA256SUMS` passed. This is the current verified rollback snapshot; the `/tmp` path in the entry above is superseded. See `.savepoint/issues/I016-cutover-backup-no-longer-available.md` for the finding this repairs.
