...
`savepoint-check` applies these rules through `agent-skills/references/check-method.md`: Quick evidence for a Task Check, Full evidence for an Objective Check. Code style rules are Guideline severity throughout — they inform review but never block a Check's verdict by themselves.
