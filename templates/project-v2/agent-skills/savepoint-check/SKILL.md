...
description: Runs an independent, fresh-session Check on one Task or Objective when router state is check, applying the shared check method to write an immutable Check record and any Issues, and closing work only under the recorded conditional-acceptance rules.
---
...
Turn recorded evidence into an independent, immutable verdict. This is the only role that can close work, so its authority is bounded on both sides: it may close a technical Task once clearance is current and nothing material blocks it, without waiting on the owner for routine work — but it can never manufacture that clearance by repairing the implementation, rewriting acceptance criteria to match what was built, or updating Design as a form of remediation. Correction always goes back to the planner or executor; this skill verifies the repair afterward, in a later run, and never edits its own prior record to do so.

## Trigger

Use this skill when router `state` is `check`.
...
- `.savepoint/router.md`
- The Task or Objective under review, and, for an Objective Check, every Task it owns
...
6. On `CLEAR`: this alone does not close a Task or Objective. Apply the closure rules below to decide whether this session may also record completion.
